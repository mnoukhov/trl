...
for split, split_name in splits_names.items():
    ...
    for batch_idx, examples in enumerate(tqdm(dataloader)):
        tokenizer.padding_side = "left"
        inputs = tokenizer(
        ...
        with torch.no_grad():
            generated_batch = generations[
                batch_idx * script_args.batch_size : (batch_idx + 1) * script_args.batch_size
            ]
            prompt_batch = prompts[batch_idx * script_args.batch_size : (batch_idx + 1) * script_args.batch_size]

            generated_texts = []
            text_with_prompt = []
            for gen_prompt, output in zip(prompt_batch, generated_batch):
                for j in range(2):  # TODO 2 generations per prompt for now - change to k
                    generated_texts.append(output.outputs[j].text)
                    text_with_prompt.append(gen_prompt + output.outputs[j].text)
            ...
            for gen_text_even, gen_text_odd, label, prompt in zip(
                ...
                if label >= 0:
                    output_dataset["chosen"].append(gen_text_even.strip())
                    output_dataset["rejected"].append(gen_text_odd.strip())
                else:
                    output_dataset["chosen"].append(gen_text_odd.strip())
                    output_dataset["rejected"].append(gen_text_even.strip())

    ds_info = DatasetInfo("CarperAI/openai_summarize_unlabelled relabeled with a DPO finetuned model")
